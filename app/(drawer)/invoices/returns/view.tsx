...
export default function ReturnsViewScreen() {
  ...
  const [iReturn, setInvoice] = useState<any>(null);
  ...
  const fetchInvoice = async (invoiceId: string) => {
    try {
      const invoiceRes = await axios.get(`${API_URL}/invoice/returns/${invoiceId}`, {
  ...
  const downloadPDF = async () => {
    try {
      Alert.alert(
        'Download PDF',
        'Do you want to download this iReturn as PDF?',
        [
            ...
            onPress: async () => {
              try {
                ...
                if (Platform.OS === 'web') {
                  ...
                  link.href = url;
                  link.download = `iReturn-${iReturn.invoice_number}.pdf`;
                  link.click();
  ...
  const emailInvoice = async () => {
    try {
      Alert.alert(
        'Email IReturn',
        'Do you want to send this iReturn via email?',
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Send Email',
            onPress: async () => {
              try {
                ...
                if (response.data.success) {
                  Alert.alert('Success', 'IReturn has been sent via email successfully.');
                } else {
      ...
      console.error('Error in email iReturn:', error);
    }
  };
    ...
    return (
      <View style={styles.safeArea}>
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorTitle}>IReturn Not Found</Text>
          <Text style={styles.errorText}>{error || 'The iReturn you are looking for does not exist.'}</Text>
          <TouchableOpacity
            style={styles.primaryButton}
            onPress={() => router.push('/(drawer)/invoices/returns/lists')}
          >
    ...
    <View style={styles.safeArea}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.invoiceHeaderCard}>
          ...
          {iReturn.description && (
            <Text style={styles.description}>{iReturn.description}</Text>
          )}
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.sectionContent}>
            <DetailItem 
              ...
              label="Customer" 
              value={iReturn.customer_name || 'N/A'} 
            />
            <DetailItem 
              icon="mail-outline" 
              label="Email" 
              value={iReturn.customer_email || 'N/A'} 
            />
            <DetailItem 
              icon="logo-whatsapp" 
              label="WhatsApp" 
              value={iReturn.customer_whatsapp || 'N/A'} 
            />
            <DetailItem 
              icon="location-outline" 
              label="Address" 
              value={iReturn.customer_address || 'N/A'} 
            />
          </View>
        </View>
