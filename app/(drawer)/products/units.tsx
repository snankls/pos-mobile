...
export default function UnitsScreen() {
  ...
  const handleSearch = (text: string) => {
    ...
    const filtered = allRecords.filter((unit) =>
      unit.name.toLowerCase().includes(text.toLowerCase())
    );
  ...
  const TableHeader = () => (
    <View style={styles.tableHeader}>
      {Object.keys(COLUMN_WIDTHS).map((key) => {
        ...
        const sortableKeys: (keyof Unit)[] = [
          'name',
          'status'
        ];

        const isSortable = sortableKeys.includes(typedKey as keyof Unit);
        ...
        return (
          ...
          >
            ...
            {isSortable && (
              <TouchableOpacity
                onPress={() => handleSort(typedKey as keyof Unit)}
                activeOpacity={0.7}
              >
                <Ionicons
                  name={
